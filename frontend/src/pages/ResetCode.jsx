      ...
      <div style={{ width:'100%', maxWidth:'360px' }}>
        ...
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'20px', padding:'28px' }}>
          ...
          <input type="text" value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,'').slice(0,6))}
            ...
            style={{ width:'100%', padding:'14px', textAlign:'center', letterSpacing:'8px', fontSize:'22px', fontWeight:700, background:'var(--bg)', border:'1px solid var(--border)', borderRadius:'12px', color:'var(--text)', outline:'none', boxSizing:'border-box', marginBottom:'16px' }} />
          <button onClick={handleSubmit} disabled={loading||code.length<6}
            style={{ width:'100%', padding:'12px', borderRadius:'12px', border:'none', cursor:'pointer', background:'linear-gradient(135deg,#7c3aed,#6d28d9)', color:'white', fontSize:'14px', fontWeight:700, opacity:loading||code.length<6?0.6:1 }}>
            {loading ? '⏳...' : 'Tasdiqlash'}
          </button>
        </div>
      </div>
