...
export default function DevPanel() {
  ...
  useEffect(() => {
    loadAll()
    timerRef.current = setInterval(loadSystem, 10000)
    return () => clearInterval(timerRef.current)
  }, [])

  const setLoad = (key, val) => setLoading(l => ({ ...l, [key]: val }))

  // ── Har bir load funksiya try/catch bilan ──────────────────
  const loadSystem = async () => {
    try { const r = await api.get('/api/dev/system');       setSystem(r.data) } catch {}
  }
  const loadDb = async () => {
    try { const r = await api.get('/api/dev/db/stats');     setDbStats(r.data) } catch {}
  }
  const loadAudit = async () => {
    try { const r = await api.get('/api/dev/audit/summary'); setAuditSum(r.data) } catch {}
  }
  const loadBlacklist = async () => {
    try { const r = await api.get('/api/dev/blacklist');    setBlacklist(r.data) } catch {}
  }
  const loadUsers = async () => {
    try { const r = await api.get('/api/dev/users/detail'); setUsers(r.data?.users || []) } catch {}
  }
  const loadApiStats = async () => {
    try { const r = await api.get('/api/dev/api-stats');    setApiStats(r.data) } catch {}
  }
  const loadRateLim = async () => {
    try { const r = await api.get('/api/dev/rate-limiter'); setRateLim(r.data) } catch {}
  }
  const loadAuditLogs = async () => {
    try { const r = await api.get('/api/dev/audit?limit=30'); setAuditLogs(r.data?.logs || []) } catch {}
  }
  ...
  const vacuumDb = async () => {
    ...
    try {
      ...
      loadDb()
    } catch { toast.error("Vacuum xato") }
    finally { setLoad('vacuum', false) }
  }

  const blockIp = async () => {
    if (!newIp.trim()) return toast.error("IP kiriting")
    try {
      ...
      loadBlacklist()
    } catch (e) { toast.error(e.response?.data?.detail || "Bloklash xato") }
  }
  ...
  const unblockIp = async (ip) => {
    try {
      ...
      loadBlacklist()
    } catch { toast.error("Blokdan chiqarish xato") }
  }
  ...
  const broadcast = async () => {
    ...
    try {
      const { data } = await api.post(
        `/api/dev/broadcast?title=${encodeURIComponent(bcTitle)}&message=${encodeURIComponent(bcMsg)}&plan=${bcPlan}`
      )
      toast.success(`${data.sent_to} ta foydalanuvchiga yuborildi`)
      setBcTitle(''); setBcMsg('')
    } catch { toast.error("Yuborish xato") }
  }

  const blacklistArr = Array.isArray(blacklist) ? blacklist : (blacklist?.blacklist || [])
    ...
    <div style={{ padding: '20px 24px', overflowY: 'auto', height: '100%' }}>
      ...
      {system && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '10px', marginBottom: '16px' }}>
          ...
          <StatCard label="RAM"      value={`${system.memory?.percent}%`}   color="#60a5fa" icon={Activity}
            sub={`${system.memory?.used_mb} / ${system.memory?.total_mb} MB`} />
          <StatCard label="Disk"     value={`${system.disk?.percent}%`}     color="#34d399" icon={Database}
            sub={`${system.disk?.free_gb} GB bo'sh`} />
          ...
          <StatCard label="Python"   value={system.python}                  color="#a78bfa" icon={Activity} />
        </div>
      )}
      ...
      {/* Rate Limiter */}
      {rateLim && (
        <Section title="Rate Limiter" icon={Activity} color="#f59e0b" defaultOpen={false}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {Object.entries(rateLim).map(([key, val]) => (
              <div key={key} style={{ padding: '5px 12px', background: 'var(--surface2)', borderRadius: '8px', fontSize: '12px' }}>
                <span style={{ color: 'var(--text3)' }}>{key}: </span>
                <span style={{ fontWeight: 700, color: '#f59e0b' }}>{typeof val === 'object' ? JSON.stringify(val) : val}</span>
              </div>
            ))}
          </div>
        </Section>
      ...
      <Section title="IP Blacklist" icon={Ban} color="#f87171">
        ...
        {blacklistArr.slice(0, 20).map(item => (
          <div key={item.id || item.ip} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', background: 'var(--surface2)', borderRadius: '9px', marginBottom: '6px', fontSize: '12px' }}>
            <div>
      ...
      <Section title="Audit Log" icon={Activity} color="#a78bfa">
        <div style={{ maxHeight: '320px', overflowY: 'auto' }}>
          ...
          {auditLogs.length === 0 && (
            <p style={{ color: 'var(--text3)', fontSize: '13px', textAlign: 'center', padding: '20px' }}>Log yo'q</p>
          )}
        </div>
      </Section>
