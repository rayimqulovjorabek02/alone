...
export default function Chat() {
  ...
  const mountedRef = useRef(true)   // StrictMode double-mount himoyasi
  ...
  useEffect(() => {
    ...
    const socket = new ChatSocket({
      onStart:  () => { if (mountedRef.current) setStreaming(true) },
      onChunk:  (content) => {
    ...
    socket.connect()
      .then(() => {
        if (mountedRef.current) {
          ...
          // StrictMode: komponent allaqachon unmount bo'lgan
          socket.disconnect()
        }
      })
      .catch(() => {
      ...
      <div style={{ width: '240px', minWidth: '240px', background: 'var(--surface)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '12px' }}>
          <button onClick={handleNewChat}
            style={{ width: '100%', padding: '9px', borderRadius: '10px', border: '1px solid var(--border)', cursor: 'pointer', background: 'var(--surface2)', color: 'var(--text2)', fontSize: '13px', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', fontFamily: 'inherit' }}>
